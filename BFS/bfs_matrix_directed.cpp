...
/*
Problem: Breadth-First Search (BFS) in a Directed Graph (Using Adjacency Matrix)
...
void bfs(int n, vector<vector<int>>& adjMatrix, int source) {
    ...
    while (!q.empty()) {
        ...
        for (int v = 0; v < n; ++v) {
            if (adjMatrix[u][v] == 1 && dist[v] == -1) {  // If v is connected and not visited
                dist[v] = dist[u] + 1;
                q.push(v);
            }
        }
...
int main() {
    ...
    bfs(n, adjMatrix, source - 1);  // Call BFS starting from the source (adjusted for 0-indexing)
